...
(function setup() {
  ...
  document.getElementById("bothswarm").addEventListener("click", bothSwarm);
  console.log("hello");
})();
...
function nodesTraverse() {
  ...
  setInterval(() => {
    ...
    // if (k < listNodes.length) listNodes[k++].classList.add("checked");
  }, 20);
}
...
// Both Swarm algorithm
let listNodes1 = [];
let listNodes2 = [];
function bothSwarm() {
  console.log("both swarm");
  let start = board.start;
  let des = board.des;
  let Q1 = [];
  let Q2 = [];
  ...
  while (Q1.length > 0 && Q2.length > 0) {
    head1 = Q1.shift();
    head2 = Q2.shift();
    if (isSamePosition(head1, head2)) return;
    let neighbours1 = list_neighbours(head1);
    for (let i = 0; i < neighbours1.length; i++) {
      Q1.push(neighbours1[i]);
      let neighbourId = `${neighbours1[i].x}-${neighbours1[i].y}`;
      let neighbour = document.getElementById(neighbourId);
      let neighbourClassList = [...neighbour.classList];
      if (
        !neighbourClassList.includes("neighbour") &&
        ...
        neighbour.classList.add("neighbour");
        listNodes1.push(neighbour);
      }
    }
    let neighbours2 = list_neighbours(head2);
    for (let i = 0; i < neighbours2.length; i++) {
      Q2.push(neighbours2[i]);
      let neighbourId = `${neighbours2[i].x}-${neighbours2[i].y}`;
      let neighbour = document.getElementById(neighbourId);
      let neighbourClassList = [...neighbour.classList];
      if (
        !neighbourClassList.includes("neighbour") &&
        ...
        neighbour.classList.add("neighbour");
        listNodes2.push(neighbour);
      }
    }
  }
  let k = 0;
  setInterval(() => {
    if (k < listNodes1.length) listNodes1[k].classList.add("check");
    if (k < listNodes2.length) listNodes2[k].classList.add("check");
    k++;
    // if (k < listNodes.length) listNodes[k++].classList.add("checked");
  }, 20);
  // while (head.parent) {
